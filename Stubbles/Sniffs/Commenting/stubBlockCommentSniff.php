 ...
 * Verifies that block comments are used appropriately.
 *
 * Copied and adapted content from Squiz_Sniffs_Commenting_BlockCommentSniff
 * because of no extending possibility (no methods/attributes to overwrite).
 *
 * @package  Commenting
 */
class Stubbles_Sniffs_Commenting_stubBlockCommentSniff implements PHP_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
         ...
         * Stubbles
         */
        // exclude Binford & stubClassLoader from comment errors
        $fullPath  = basename($phpcsFile->getFilename());
        $className = substr($fullPath, 0, strrpos($fullPath, '.'));
        if (stubClassExcluder::isExcluded($className) === true) {
            return;
        }
        ...
        if (count($commentLines) <= 2) {
            ...
            if (count($commentLines) === 1) {
                // Comment of type "/* @var  $foo  SomeClass */" ?
                if (strstr($tokens[$commentLines[0]]['content'], '@var') !== false) {
                    // ignore "/* @var  $foo  SomeClass */"
                    return;
                }
